...
import jp.co.sss.cytech.web.dto.RegisterForm; import jakarta.validation.Valid;
...
public class AuthController {
    ...
    @GetMapping("/register") public String register(@ModelAttribute RegisterForm f){ return "auth/register"; }
    @PostMapping("/register/submit")
    public String submit(@Valid @ModelAttribute RegisterForm f, BindingResult br, RedirectAttributes ra){
        if(!f.getPassword().equals(f.getConfirmPassword())){ br.rejectValue("confirmPassword","mismatch","確認用パスワードが一致しません"); }
        if(br.hasErrors()) return "auth/register";
        User u = new User(); u.setUserName(f.getUserName()); u.setUserNameKana(f.getUserNameKana());
        u.setEmail(f.getEmail()); u.setPhone(f.getPhone()); u.setUserAddress(f.getUserAddress());
        u.setPassword(encoder.encode(f.getPassword())); users.save(u);
        ra.addFlashAttribute("registered", true); return "redirect:/auth/login";
    }
}
